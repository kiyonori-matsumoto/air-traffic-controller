...
export class Game extends Scene {
    ...
    if (this.runwayVisuals.length === 0 && this.airport) {
      this.airport.runways.forEach((rwy) => {
        ...
        const beam = this.add.triangle(
          ...
          0,
          0,
          Math.sin(((beamAngle - 3) * Math.PI) / 180) * beamLength,
          -Math.cos(((beamAngle - 3) * Math.PI) / 180) * beamLength,
          Math.sin(((beamAngle + 3) * Math.PI) / 180) * beamLength,
          -Math.cos(((beamAngle + 3) * Math.PI) / 180) * beamLength,
          0x00ffff,
          0.1,
  ...
  private handleCommand(cmd: string) {
    ...
    if (cmd === "CONTACT TOWER" || cmd === "CT") {
      ...
      if (dist >= 10 || ac.state !== "LANDING") {
        ...
        console.log(
          `${ac.callsign} Unable to contact tower (Dist: ${dist.toFixed(1)}NM, State: ${ac.state})`,
        );
        return;
      }
    }
  ...
  private schedulePilotResponse(
    ac: Aircraft,
    msg: string,
    onValidReadback?: () => void,
  ) {
  ...
  update(time: number, delta: number) {
    ...
    this.radar.scan(
      this.trafficManager.getAircraftsLogic(),
      radarUpdate.prevAngle,
      radarUpdate.currentAngle,
    );
